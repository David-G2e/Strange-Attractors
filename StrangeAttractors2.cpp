...
void* Animation::animationThreadMethod(void)
	...
	while(true)
		...
		/* Check the foreground thread for new inputs: */
		while (pvQ != NULL)
			{
				pv.enqueue();
				pvQ.dequeue();
		...
		// if the foreground thread currently has an input: */
		if ()
			{
				/*enqueue function takes the x,y,z as a parameter,
				creates a new particle with random color, adds it to pvQ */
				pvQ.enqueue();
			}
...
Animation::Animation(int& argc,char**& argv)
	...
	/* new Queue interacting with the foreground thread: */
	int particleSizeQueue = 0;
	ParticleVertex* pvQ=new ParticleVertex[particleSizeQueue];
	}
